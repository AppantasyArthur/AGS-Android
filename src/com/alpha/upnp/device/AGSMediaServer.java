...
public class AGSMediaServer {
	
	//private SimpleWebServer msDemand = null;

	private final LocalDevice device;
	private final LocalServiceBinder binder = new AnnotationLocalServiceBinder();
	
	private final String SYSTEM_ID = "Alpha AGS Media Server";
	private final String UDA_DEVICE_TYPE = "MediaServer";
	private final String NAME = "";
	
	private final ServiceManager<AGSMediaServerContentDirectoryService> avTransport;
	
	public AGSMediaServer(){
		
		  //The AVTransport just passes the calls on to the backend players
        LocalService<AGSMediaServerContentDirectoryService> avTransportService = binder.read(AGSMediaServerContentDirectoryService.class);
        avTransport =
                new DefaultServiceManager<AGSMediaServerContentDirectoryService>(avTransportService) {
                    @Override
                    protected AGSMediaServerContentDirectoryService createServiceInstance() throws Exception {
                        return new AGSMediaServerContentDirectoryService();
                    }
                };
		...
		try {
            ...
            device = new LocalDevice(
                    new DeviceIdentity(UDN.uniqueSystemIdentifier(SYSTEM_ID)),
                    new UDADeviceType(UDA_DEVICE_TYPE, 1),
                    new DeviceDetails(
                    		SYSTEM_ID,
                            new ManufacturerDetails("Alpha Networks", "http://www.alphanetworks.com/"),
                            new ModelDetails(SYSTEM_ID, NAME, "1", "http://www.alphanetworks.com/mediarender")
                    ),
        ...
        //File wwwroot = new File(AGSMediaServerContentDirectoryService.ROOT);
...
//	    	msDemand = new MediaServerDemand(formatedIpAddress, port);
//	    	try {
//				msDemand.start();
